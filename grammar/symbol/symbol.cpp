...
SMB::FuncSymbol::FuncSymbol(ASTREE::RootNode* node) {
    ASTREE::DefineFunctionNode* tmp_node = (ASTREE::DefineFunctionNode*)node;
    this->dec_name = tmp_node->getContent();
    this->arg_list =tmp_node->getArgList();
    this->rtn_type = tmp_node->getReturnSymbolType();

    ASTREE::DefineVarNode* p = (ASTREE::DefineVarNode*)this->arg_list;
    while(p){
        ...
        total_arg_offset += 4;
        p = (ASTREE::DefineVarNode*)(p->getPeerNode());
    }
}
...
SMB::StructSymbol::StructSymbol(std::string name, ASTREE::RootNode* node){
    ...
    while(curr_node){
        ...
        offset+=4;
        curr_node = curr_node->getPeerNode();
    }
...
void SMB::SymbolTable::addFromFunctionArgs(FuncSymbol *func_node) {
    ...
    int offset = -4;
    int index = -1;
    std::stack<ASTREE::RootNode*> tmp_stack;
    while (args) {
        tmp_stack.push(args);
    ...
    while (tmp_stack.size()) {
        // this->addSymbol(args);
        ASTREE::DefineVarNode *arg = (ASTREE::DefineVarNode *)tmp_stack.top();
...
int SMB::SymbolTable::addArraySymbol(ASTREE::RootNode *array_node){
    ASTREE::DefineVarNode *curr_array = (ASTREE::DefineVarNode *)array_node;
